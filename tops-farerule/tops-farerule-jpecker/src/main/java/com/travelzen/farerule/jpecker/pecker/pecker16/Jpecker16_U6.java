...
public class Jpecker16_U6 extends Jpecker16__Base {

	public void process(String ruleText) {
		
		Matcher matcher1 = Pattern.compile(
				"\nIF INFANT (?:UNDER|0)[\\w\\W]+?"
				+ "\nOTHERWISE\n([\\w\\W]+)$").matcher(ruleText);
		Matcher matcher2 = Pattern.compile(
				"^([\\w\\W]+?)"
				+ "\nIF INFANT (?:UNDER|0)").matcher(ruleText);
		if (matcher1.find()) {
			Jpecker16__General general = new Jpecker16__General();
			general.process("U6", matcher1.group(1));
			penalties = general.getPenalties();
		} else if (matcher2.find()) {
			Jpecker16__General general = new Jpecker16__General();
			general.process("U6", matcher2.group(1));
			penalties = general.getPenalties();
		} else {
			Jpecker16__General general = new Jpecker16__General();
			general.process("U6", ruleText);
			penalties = general.getPenalties();
		}
	}
}