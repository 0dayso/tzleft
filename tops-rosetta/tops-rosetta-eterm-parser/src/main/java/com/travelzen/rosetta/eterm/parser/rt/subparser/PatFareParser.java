...
public enum PatFareParser {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(PatFareParser.class);
	...
	public static PatFare parseGov(String pat) {
		...
		if (fareItems.size() == 0) {
			LOGGER.error("PAT政府报价解析失败！解析文本：{}", pat);
			return null;
		}
	...
	public static PatFare parseTrd(String pat) {
		if (pat.startsWith("  NO ACTIVE")){
			LOGGER.info("PAT:A解析 NO ACTIVE");
			return null;
		}
		...
		PatFare fare = new PatFare(PassengerType.TRD);
		List<PatFareItem> fareItems = new ArrayList<PatFareItem>();
		Matcher matcher = pattern_segment.matcher(pat);
		while (matcher.find()) {
			PatFareItem fareItem = parseAdtItem(matcher.group(1));
		...
		if (fareItems.size() == 0) {
			LOGGER.error("PAT三方报价解析失败！解析文本：{}", pat);
			return null;
		}
		fare.setFareItems(fareItems);
		return fare;
	...
	public static PatFare parseAdt(String pat) {
		...
		if (fareItems.size() == 0) {
			LOGGER.error("PAT:A解析失败！解析文本：{}", pat);
			return null;
		}
		fare.setFareItems(fareItems);
		return fare;
	...
	private static PatFareItem parseAdtItem(String patItem) {
		...
		if (matcher_fareBasis.find() && matcher_fare.find() && matcher_total.find()) {
			...
			LOGGER.debug("PAT:A segment解析失败！解析文本：{}", patItem);
			return null;
		}
		if (matcher_tax.find())
	...
	public static PatFare parseChd(String patChd) {
		if (patChd.startsWith("  NO ACTIVE")){
			LOGGER.debug("PAT:A解析 NO ACTIVE");
			return null;
		}
		if (patChd.startsWith("  NO PNR")){
			LOGGER.debug("PAT:A解析 NO PNR");
			return null;
		}
		if (patChd.startsWith("  CAN NOT USE *CH")){
			LOGGER.debug("PAT:A解析 CAN NOT USE *CH");
			return null;
		}
		...
		if (fareItems.size() == 0) {
			LOGGER.debug("PAT:A*CH解析失败！解析文本：{}", patChd);
			return null;
		}
		fare.setFareItems(fareItems);
		return fare;
	...
	private static PatFareItem parseChdItem(String patChdItem) {
		...
		if (matcher_fareBasis.find() && matcher_fare.find() && matcher_total.find()) {
			...
			LOGGER.debug("PAT:A*CH segment解析失败！解析文本：{}", patChdItem);
			return null;
		}
		if (matcher_sfc.find())
			fareItem.setSfc(matcher_sfc.group(1));
	...
	public static PatFare parseInf(String patInf) {
		...
		if (fareItems.size() == 0) {
			LOGGER.debug("PAT:A*IN解析失败！解析文本：{}", patInf);
			return null;
		}
		fare.setFareItems(fareItems);
		return fare;
	...
	private static PatFareItem parseInfItem(String patInfItem) {
		...
		if (matcher_fareBasis.find() && matcher_fare.find() && matcher_total.find()) {
			...
			LOGGER.debug("PAT:A*IN segment解析失败！解析文本：{}", patInfItem);
			return null;
		}
		if (matcher_sfc.find())
			fareItem.setSfc(matcher_sfc.group(1));
